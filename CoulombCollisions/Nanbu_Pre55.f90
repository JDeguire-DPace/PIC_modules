...
program CoulombCollisions
    ...
    real(kind=8), parameter :: density_ions = 1.d18 ! Change Delta_t to check convergence
    real(kind=8), parameter :: mass_electrons = 9.10938356d-31, elementary_charge = 1.6021766209d-19
    real(kind=8), parameter :: mass_ion = 1836.d0 * mass_electrons
    real(kind=8), parameter :: charge_electrons = -1.d0*elementary_charge
    real(kind=8), parameter :: charge_ions = real(Z_ion,8) * elementary_charge
    real(kind=8), parameter :: e = dexp(1.d0), epsilon0 = 8.854187817d-12, density_electrons = real(Z_ion,8) * density_ions, k_B = 1.380649d-23
    real(kind=8), parameter :: T0 = 10.d0 * elementary_charge

    ! Derived parameters
    real(kind=8), parameter :: Initial_energy_electrons = T0
    real(kind=8), parameter :: Initial_energy_ions = T0
    ...
    real(kind=8), parameter :: debye_length = dsqrt(epsilon0*(2.d0*T0) / (density_ions*charge_ions**2 + density_electrons*charge_electrons**2)) ! Debye length
    real(kind=8), parameter :: b_min = abs(charge_electrons*charge_ions) / (4.d0 * pi * epsilon0 * (2.d0*T0)) ! Minimum impact parameter
    real(kind=8), parameter :: CoulombLog = dlog(debye_length/b_min) ! Coulomb logarithm
    real(kind=8) ,parameter :: tau0 = (5.d0/(8.d0*dsqrt(2.d0*pi)))*8.d0*pi*dsqrt(2.d0)*epsilon0**2*dsqrt(mass_electrons)*T0**1.5/(density_ions*elementary_charge**4*CoulombLog) ! Characteristic time for the collision

    real(kind=8), parameter :: Delta_t = 0.04d0*tau0
    real(kind=8), parameter :: t_final = 11.d0*tau0
    ...
    integer :: i, j, i_particle, i_tstep, number_time_step = t_final/Delta_t
    real(kind=8) :: time
    ...
    ! Open output file
    open(10, file="Flow_Pre55.txt", status="replace") ! Adapt title to compare different dt's
    ...
    do i_tstep = 0, number_time_step
        ...
        if (i_tstep == 0) then
            ...
            do i = 1, Number_ions    
                ...
                array_ions(i)%velocity = maxwell_boltzmann(0.d0, dsqrt(3.d0 * T0 / mass_ion), random_Gauss_ion)
            end do
            ! do i = 1, Number_electrons
            !     do j = 1, 3
            !         call rran2(random_Gauss_elec(j), i_seed)
            !     end do
            !     array_electrons(i)%velocity = maxwell_boltzmann(Flow_elec, dsqrt(3.d0 * Temp_elec * elementary_charge / mass_electrons), random_Gauss_elec)
            ! enddo
        end if
        ! Reset averages
        averageV_ele = 0.d0
        ...
        write(10, '(5EN14.4)') time/tau0, Temp_elec, Temp_ion, Flow_elec, Flow_ion ! Write to file
    end do
